...
import { 
  ...
  Platform // <--- 2. Importado para diferenciar iOS/Android
} from 'react-native';
...
const AnnouncementDetail: React.FC<AnnouncementDetailProps> = ({ announcement, onBack, onStatusUpdate }) => {
  ...
  return (
    // <--- 3. Envolvendo tudo no KeyboardAvoidingView
    <KeyboardAvoidingView 
      style={{ flex: 1 }} 
      behavior={Platform.OS === 'ios' ? 'padding' : 'height'}
      keyboardVerticalOffset={Platform.OS === 'ios' ? 0 : 0} // Ajuste se tiver header fixo fora do scroll
    >
      <ScrollView 
        className="flex-1 bg-gray-50"
        keyboardShouldPersistTaps="handled" // <--- 4. Importante para o botão funcionar no primeiro toque
